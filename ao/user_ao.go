...
func UpdateScore(pa *do.GameUserInfo) (*do.GameUserInfo, error) {

	affect, err := do.UpdateUser(&models.TGameUserinfo{
		Name:   pa.Name,
		Email:  pa.Email,
		Passwd: pa.Passwd,
		Score:  pa.Score,
	}, &models.TGameUserinfo{
		Name:   pa.Name,
		Email:  pa.Email,
		Passwd: pa.Passwd,
	})

	fmt.Printf("----------user_ao UpdateScore affect: %v-------------------\n", affect)
	fmt.Printf("----------user_ao UpdateScore err: %v-------------------\n", err)

	if err != nil {
		log.WithFields(log.Fields{
			"event": "Update",
			"desc":  "Game Update Failed",
		}).Warnf("%s,%v\n", pa, err)
		return nil, err
	}
