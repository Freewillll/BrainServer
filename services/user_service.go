...
func (pa *UpdateScoreParam) String() string {
	jsonres, err := json.Marshal(pa)
	if err != nil {
		return ""
	}
...
func (pa *UpdateScoreParam) FromJsonString(jsonstr string) (utils.RequestParam, error) {
	if err := json.Unmarshal([]byte(jsonstr), pa); err != nil {
		return nil, err
	}
	return pa, nil
...
func SetUserScore(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("----------update decode error: %v-------------------\n", err)
	_, ok := param.(*UpdateScoreParam)
	fmt.Printf("----------update param transfer error: %v-------------------\n", ok)

	if !ok {
		log.WithFields(log.Fields{
			"event": "Game update",
			"desc":  "Game param.(*do.updateUserInfo) failed",
		}).Errorf("%v\n", err)
		utils.EncodeToHttp(w, 400, "Bad Request")
		return
	}
	...
	userinfo, err := ao.UpdateScore(&do.GameUserInfo{
		Name:   p.User.Name,
		Email:  p.User.Email,
		Passwd: p.User.Passwd,
		Score:  p.Score,
	...
	fmt.Printf("----------update user info: %v-------------------\n", userinfo)
	fmt.Printf("----------update ao.UpdateScore error: %v-------------------\n", err)
